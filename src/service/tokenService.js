...
export async function tokenCreate(token) {
  ...
  try {
    ...
    if (transactionReceipt.status !== Status.Success) {
      ...
      //************************************************************************ */
      // automatically associate, grant, etc... for Alice
      tokenAssociate(token.tokenId, "Alice").then(() => {
        const aliceAccountId = getAccountDetails("Alice").accountId;
        notifySuccess("token association with Alice successful");
        if (token.kycKey) {
          const instruction = {
            tokenId: token.tokenId,
      ...
      // automatically associate, grant, etc... for Bob
      tokenAssociate(token.tokenId, "Bob").then(() => {
        const bobAccountId = getAccountDetails("Bob").accountId;
        notifySuccess("token association with Bob successful");
        if (token.kycKey) {
          const instruction = {
            tokenId: token.tokenId,
